#!/bin/bash
# 🚀 Script de Gestion Infrastructure veza avec Incus (Réseau par défaut)
...
NETWORK_NAME="incusbr0"  # Utilise le réseau par défaut
...
setup_network() {
    log "Vérification du réseau par défaut $NETWORK_NAME..."
    
    if incus network show "$NETWORK_NAME" &>/dev/null; then
        success "Réseau par défaut disponible"
    else
        error "Réseau par défaut $NETWORK_NAME non disponible"
    fi
}
...
create_base_container() {
    ...
    # Créer le container avec le réseau par défaut
    incus launch images:ubuntu/22.04 "$name" --network "$NETWORK_NAME"
